...
export class LocalStorage implements IStorage {
  private getStorageKey(prefix: string, userName: string, key?: string): string {
    if (key) {
      return `gougoutv_${prefix}_${userName}_${key}`;
    }
    return `gougoutv_${prefix}_${userName}`;
  }
  ...
  async setAdminConfig(config: AdminConfig): Promise<void> {
    ...
    try {
      localStorage.setItem('gougoutv_admin_config', JSON.stringify(config));
    } catch (error) {
