...
// 从localStorage读取管理员配置（客户端专用）
function getAdminConfigFromLocalStorage(): AdminConfig | null {
  if (typeof window === 'undefined') return null;

  try {
    const data = localStorage.getItem('gougoutv_admin_config');
    return data ? JSON.parse(data) : null;
  } catch (error) {
    console.error('Error getting admin config from localStorage:', error);
    return null;
  }
}
  ...
  if (storageType !== 'localstorage') {
    ...
    try {
      ...
      if (adminConfig) {
        ...
        adminConfig = {
          SiteConfig: {
            SiteName: process.env.SITE_NAME || 'GouGouTV',
            Announcement:
    ...
    // 本地存储模式：先尝试从localStorage读取管理员配置
    const localStorageConfig = getAdminConfigFromLocalStorage();

    if (localStorageConfig) {
      // 使用localStorage中的配置
      cachedConfig = localStorageConfig;
    } else {
      // 本地存储直接使用文件配置
      cachedConfig = {
        SiteConfig: {
...
export async function getConfig(): Promise<AdminConfig> {
  ...
  try {
    ...
    if (adminConfig) {
      // 合并一些环境变量配置
      adminConfig.SiteConfig.SiteName = process.env.SITE_NAME || 'GouGouTV';
