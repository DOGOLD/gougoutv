...
export async function GET(request: Request) {
  ...
  try {
    // 检查是否明确要求过滤成人内容
    const filterAdultParam = searchParams.get('filter_adult') === 'true';

    // 获取用户的成人内容过滤设置
    let shouldFilterAdult = false; // 默认不过滤
    if (userName && filterAdultParam) {
      try {
        const storage = getStorage();
        const userSettings = await storage.getUserSettings(userName);
        // 如果用户设置存在且明确设为true，则过滤
        shouldFilterAdult = userSettings?.filter_adult_content === true;
      } catch (error) {
        // 出错时默认不过滤
    ...
    // 根据用户设置决定最终的过滤策略
    const finalShouldFilter = shouldFilterAdult;
